...
class MaterialDayPicker @JvmOverloads constructor(
    ...
    // Constants
    private val SIZE_SMALL = 0
    private val SIZE_MEDIUM = 1
    ...
    /**
     * Set day picker size mode.
     */
    var dayPickerSize: Int = SIZE_MEDIUM
    ...
    private var firstDayOfWeek: Weekday = Weekday.MONDAY
    ...
    /**
     * Returns an int bitmask of the currently selected [Weekday]s
     */
    fun getSelectedDaysAsBooleanArray(): BooleanArray {
        val days = BooleanArray(8)

        for((i, day) in Weekday.allDays.asReversed().withIndex()) {
            days[0] = false
            if(isSelected(day)) {
                days[i+1] = true
            }
    ...
    fun setSelectedDaysFromBooleanArray(selectedDaysBooleanArray: BooleanArray) {
        clearSelection()

        for ((index, day) in selectedDaysBooleanArray.reversed().withIndex()) {
            if(day && index < 7) {
                selectDay(Weekday[index])
            }
        }
    }
    ...
    private fun inflateLayoutUsing(context: Context) {
        if(dayPickerSize == SIZE_SMALL) {
            LayoutInflater.from(context).inflate(R.layout.day_of_the_week_picker_small, this, true)
        }
        if(dayPickerSize == SIZE_MEDIUM) {
            LayoutInflater.from(context).inflate(R.layout.day_of_the_week_picker_medium, this, true)
        }
    }
...
//        }
        ...
        dayPickerSize = typedAttributeArray.getInt(R.styleable.MaterialDayPicker_dayPickerSize, SIZE_MEDIUM);
    ...
    private fun createSelectionMode(className: String): SelectionMode {
        val selectionModeClass = try {
            ...
            throw IllegalArgumentException("Cannot find class for SelectionMode named '$className' set via xml. Make sure you are specifying the correct fully qualified class name (i.e com.touchip.organizer.views.datapicker.materialdaypicker.SingleSelectionMode).")
        }
    ...
    private fun bindViews() {
        ...
        dayToggles.apply {
            ...
            else if (dayPickerSize == SIZE_MEDIUM) {
                add(toggle_0)
                add(toggle_1)
                add(toggle_2)
                add(toggle_3)
    ...
    private fun getTextSizeForLocale(locale: Locale): Float {
        var toggleButtonWidth = resources.getDimension(R.dimen.day_button_size)
        var maxTextSize = resources.getDimension(R.dimen.day_button_max_font_size)

        if(dayPickerSize == SIZE_SMALL) {
            toggleButtonWidth = resources.getDimension(R.dimen.day_small_button_size)
            maxTextSize = resources.getDimension(R.dimen.day_button_small_font_size)
        }
