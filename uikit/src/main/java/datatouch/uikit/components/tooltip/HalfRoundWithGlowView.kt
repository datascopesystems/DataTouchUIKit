...
class HalfRoundWithGlowView : View {
    ...
    private fun parseNativeAttributes(attrs: AttributeSet?) {
        ...
        try {
            layoutWidth =
                typedArray.getLayoutDimension(0, DEFAULT_WIDTH)
            layoutHeight =
    ...
    override fun setLayoutParams(params: ViewGroup.LayoutParams) {
        ...
        if (params != null) {
            layoutWidth =
                if (params.width < 0) DEFAULT_WIDTH else params.width
            layoutHeight =
                if (params.height < 0) DEFAULT_HEIGHT else params.height
        }
    }
    ...
    private fun drawAnchorArc(
        ...
        val rect = RectF(
            ...
            0F,
            (layoutWidth / RECT_WIDTH_COEF).toFloat(),
            layoutHeight.toFloat()
        )
        canvas.drawArc(
            rect,
    ...
    companion object {
        private const val RECT_WIDTH_COEF = 4
        private const val ARC_START_ANGLE = 310
        private const val ARC_SWEEP_ANGLE = 130
        private const val BLUR_RADIUS_COEF = 7
