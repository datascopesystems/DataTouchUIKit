...
import datatouch.uikit.components.camera.CameraViewExtensions.FLAGS_FULLSCREEN
...
const val CameraPhotoActivityParamsKey = "CameraPhotoActivityParamsKey"
...
class CameraPhotoActivity : AppCompatActivity() {
    ...
    private lateinit var activityParams: CameraPhotoActivityParams
    ...
    private fun injectActivityParams() {
        val params = intent.extras?.getSerializable(CameraPhotoActivityParamsKey) as? CameraPhotoActivityParams?
        params?.let { activityParams = it } ?: run {
            activityParams = CameraPhotoActivityParams(filesDir)
        }
    }
                ...
                imageCapture.takePicture(
                    outputOptions, executor, object : ImageCapture.OnImageSavedCallback {
                        ...
                        override fun onImageSaved(output: ImageCapture.OutputFileResults) {
                            onImageSaved(newPhotoJpgFile, output.savedUri)
                        }
    ...
    private fun onImageSaved(photoFile: File, photoUri: Uri?) {
        val savedUri = photoUri ?: Uri.fromFile(photoFile)
    ...
    private fun applyUserConfiguration() {
        ...
        if (true == getConfigurationValue(CameraConfiguration.FULL_SCREEN_ENABLED)) {
            ...
            rootLayout?.postDelayed({
                rootLayout?.systemUiVisibility = FLAGS_FULLSCREEN
